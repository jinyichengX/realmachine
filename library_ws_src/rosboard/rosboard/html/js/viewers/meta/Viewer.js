...
class Viewer {
  ...
  **/
  ...
  drawbox(x, y, w, h, status) {
    ...
    this.selectBox.css({"width": w + "px"});
    this.selectBox.css({"height": h + "px"});
    if (status == 0) {
      this.selectBox.css({"display": "none"});
      this.selectBox = null;
    }
    else {
      this.selectBox.css({"display": ""});
      if (status == 1) {
        this.selectBox.css({"right": "auto"});
        this.selectBox.css({"bottom": "auto"});
        this.selectBox.css({"left": x + "px"});
        this.selectBox.css({"top": y + "px"});
      }
      else if (status == 2) {
        this.selectBox.css({"right": "auto"});
        this.selectBox.css({"top": "auto"});
        this.selectBox.css({"left": x + "px"});
        this.selectBox.css({"bottom": y + "px"});
      }
      else if (status == 3) {
        this.selectBox.css({"bottom": "auto"});
        this.selectBox.css({"left": "auto"});
        this.selectBox.css({"right": x + "px"});
        this.selectBox.css({"top": y + "px"});
      }
      else if (status == 4) {
        this.selectBox.css({"top": "auto"});
  ...
  status(status_text) {
    ...
    let that = this;
    this.statusBox.css({"display": ""});
    this.statusHideTimeout = setTimeout(() => that.statusBox.css({"display": "none"}), 1000);
    this.statusBox.text(status_text);
  }
}
