...
class color_follow:
    ...
    def object_follow(self, img, hsv_msg):
        ...
        if len(contours) != 0:
            ...
            self.Center_y = 0
            self.Center_r = 0
        HSV_min = 'HSV_min : (' + str(lower[0]) + ' ,' + str(lower[1]) + ' ,' + str(lower[2]) + ')'
        HSV_max = 'HSV_max : (' + str(upper[0]) + ' ,' + str(upper[1]) + ' ,' + str(upper[2]) + ')'
        if lower[1] < 5 or lower[2] < 5:
            cv.putText(img, 'Learning...', (30, 50), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
            cv.putText(img, HSV_min, (180, 30), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
            cv.putText(img, HSV_max, (180, 60), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
        else:
            cv.putText(img, 'Success!!!', (30, 50), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
            cv.putText(img, HSV_min, (180, 30), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
            cv.putText(img, HSV_max, (180, 60), cv.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
    ...
    def Roi_hsv(self, img, Roi):
        ...
        # HSV_min = 'HSV_min : (' + str(H_min) + ' ,' + str(S_min) + ' ,' + str(V_min) + ')'
        # HSV_max = 'HSV_max : (' + str(H_max) + ' ,' + str(S_max) + ' ,' + str(V_max) + ')'
